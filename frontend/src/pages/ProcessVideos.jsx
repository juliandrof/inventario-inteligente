          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 12 }}>
            ...
            <div onClick={() => setStep('upload')}
              ...
              <div style={{ marginBottom: 12 }}>
                <svg width="48" height="48" viewBox="0 0 48 48" fill="none"><path d="M24 6v28M24 6l-10 10M24 6l10 10M6 34v6a2 2 0 002 2h32a2 2 0 002-2v-6" stroke="var(--dbxsc-primary)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </div>
              <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 6 }}>{t('process.upload_title')}</div>
              <p style={{ fontSize: 13, color: '#999' }}>{t('process.upload_desc')}</p>
            </div>
            {/* Batch option */}
            <div onClick={() => setStep('batch')}
              style={{ padding: 24, borderRadius: 12, border: '2px solid #e0e0e0', cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--dbxsc-primary)'; e.currentTarget.style.transform = 'scale(1.02)'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = '#e0e0e0'; e.currentTarget.style.transform = 'scale(1)'; }}>
              <div style={{ marginBottom: 12 }}>
                <svg width="48" height="48" viewBox="0 0 48 48" fill="none"><rect x="4" y="4" width="16" height="16" rx="3" fill="var(--dbxsc-primary)" opacity="0.7"/><rect x="28" y="4" width="16" height="16" rx="3" fill="var(--dbxsc-primary)" opacity="0.5"/><rect x="4" y="28" width="16" height="16" rx="3" fill="var(--dbxsc-primary)" opacity="0.5"/><rect x="28" y="28" width="16" height="16" rx="3" fill="var(--dbxsc-primary)" opacity="0.3"/></svg>
              </div>
              <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 6 }}>{t('process.batch_title')}</div>
              <p style={{ fontSize: 13, color: '#999' }}>{t('process.batch_desc')}</p>
            </div>
          </div>
        </div>
      </div>
